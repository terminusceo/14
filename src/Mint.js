...
function Mint() {
    ...
    const [feedback, setFeedback] = useState('Click to mint your ETHERINGS.');
    ...
    const endTime = new Date('2023-08-15T08:35:00Z');
            ...
            {isWalletConnected ? (
                <>
                    <AllocationContainer>
                        ...
                        <ListText>Ringlist: 0</ListText>
                        <ListText>Waitlist: 0</ListText>
                        <LimitText>Public mint limit: 5</LimitText>
                        <ListText>Remaining Supply: {CONFIG.MAX_SUPPLY - data.totalSupply}</ListText>
                    </AllocationContainer>
                    ...
                    <StyledButton onClick={claimNFTs} disabled={!mintOpen || claimingNft}>
                        {claimingNft ? 'Minting...' : `Mint ${mintAmount} for ${(CONFIG.DISPLAY_COST * mintAmount).toFixed(1)} ETH`}
                    </StyledButton>
                </>
            ) : (
                <WhiteText>Please connect your wallet to mint ETHERINGS.</WhiteText>
            )}
        </MintContainer>
    );
}
